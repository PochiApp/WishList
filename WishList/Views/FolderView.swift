...
struct FolderView: View {
    ...
    @FetchRequest(
        ...
        sortDescriptors: [
            NSSortDescriptor(keyPath: \FolderModel.folderIndex, ascending: true)
        ],
        animation: .default
    )
...
extension FolderView {
            ...
            ForEach(folderModel) { foldermodel in
                NavigationLink(
                    destination: ListView(
                        ...
                        isInsertPassViewBeforeListView:
                            $isInsertPassViewBeforeListView
                    )
                ) {
                    RoundedRectangle(cornerRadius: 16)
                        ...
                        .overlay(
                            LazyVStack(alignment: .center) {
                                ...
                                //リスト達成率表示部分
                                if let listsAchieved = foldermodel
                                    .achievedLists,
                ...
                .contextMenu(
                    ContextMenu(menuItems: {
                        ...
                        Button(
                            action: {
                                wishListViewModel.editFolder(
                                    updateFolder: foldermodel
                                )
                        ...
                        .sheet(isPresented: $isShowAddAndEditFolderView) {
                            AddAndEditFolderView(
                                wishListViewModel: wishListViewModel,
                                isShowAddAndEditFolderView:
                                    $isShowAddAndEditFolderView
                            )
                ...
                .onDrag {
                    ...
                    let draggedFolder = String(
                        currentFolder?.folderIndex ?? Int16(0)
                    )
                ...
                .onDrop(
                    ...
                    delegate: DropViewDelegate(
                        dropFolder: foldermodel,
                        folders: folderModel,
                        currentDragFolder: currentFolder,
                        context: context,
                        wishListViewModel: wishListViewModel
                    )
                )
                //パスコード設定画面の表示
                .fullScreenCover(
                    isPresented: $isShowSetPassView,
                    content: {
                        SetPassView(
                            wishListViewModel: wishListViewModel,
                            isShowSetPassView: $isShowSetPassView,
                            isShowUnlockPassView: $isShowUnlockPassView
                        )
                    }
                )
                ...
                .fullScreenCover(
                    isPresented: $isShowUnlockPassView,
                    content: {
                        SetPassView(
                            wishListViewModel: wishListViewModel,
                            isShowSetPassView: $isShowSetPassView,
    ...
    private var floatingButton: some View {
        ...
        .sheet(isPresented: $isShowAddAndEditFolderView) {
            AddAndEditFolderView(
                wishListViewModel: wishListViewModel,
                isShowAddAndEditFolderView: $isShowAddAndEditFolderView
            )
